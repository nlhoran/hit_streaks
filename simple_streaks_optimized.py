...
# Modified to reduce API calls and handle timeouts better
...
# Modified to improve performance and handle timeouts
...
# Function to update streak data from a file (for external updates)
def load_streak_data_from_file():
    """Load streak data from a JSON file if it exists"""
    data_file = os.path.join(CACHE_DIR, "current_streaks.json")
...
data_source = st.sidebar.radio(
    "Data Source",
    options=["Current MLB Data", "Demo Data"],
    index=0
)
...
with tab1:
    ...
    # Load data based on selection, with session state caching
    if data_source == "Current MLB Data":
...
# Footer
st.markdown("---")
st.markdown("Data updates hourly or when refresh button is clicked. Last update: " + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))